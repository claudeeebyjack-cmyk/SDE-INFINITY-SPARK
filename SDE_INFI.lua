...
RunService.Heartbeat:Connect(function(dt)
	...
	if HB.crash >= 0.05                  then HB.crash = 0; _SDE.hbCrash() end
	if HB.buf   >= (1/SPA_Replay.BUF_HZ) then HB.buf   = 0; _SDE.hbBuf()   end
	if HB.prx   >= 0.1                   then HB.prx   = 0; _SDE.hbPrx()   end
	if HB.head  >= 0.06                  then HB.head  = 0; _SDE.hbHead()  end
	if HB.lap   >= 0.06                  then HB.lap   = 0; _SDE.hbLap()   end
	if HB.anal  >= 0.5                   then HB.anal  = 0; _SDE.hbAnal()  end
	if HB.tire  >= 0.5                   then HB.tire  = 0; _SDE.hbTire()  end
end)
