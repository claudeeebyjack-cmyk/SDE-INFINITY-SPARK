...
function _setupUI2()  -- [SPAV4] global: libera registros del ámbito principal
	...
	_SDEI.hbHead = function()
		-- [FIX] Eliminado el gate de localChar — bloqueaba el loop entero si el operador
		-- no tenía personaje cargado. El SpeedTag se muestra a TODOS independientemente.
		for _, p in ipairs(Players:GetPlayers()) do
			local char = p.Character
			if not char then continue end
			...
			local inVehicle = seat and (seat:IsA("VehicleSeat") or seat:IsA("Seat")) and seat.Occupant == humanoid
			...
			-- Nombre — siempre visible si SHOW_HEAD_NAME
			...
			if inVehicle then
				-- ── En vehículo: mostrar speed + telemetría ──────────
				if speedLbl then
					local uid    = p.UserId
					...
					-- Alerta de motor ilegal — una sola vez por cambio de MaxSpeed
					if maxSpd > effLim and lastSpeeds[p.UserId] ~= maxSpd then
						showSpeedingNotification(getDisplayName(p), maxSpd)
					end
					lastSpeeds[p.UserId] = maxSpd
				end
				...
				-- ── Fuera de vehículo: limpiar líneas de datos ───────
