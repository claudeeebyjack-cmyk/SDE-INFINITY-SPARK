...
local function getPlayerSpeedLimit(seat)
	...
	-- Fallback: propiedad nativa MaxSpeed del VehicleSeat (Seat normal no la tiene)
	if seat:IsA("VehicleSeat") then
		return mfloor(seat.MaxSpeed * 10 + 0.5) / 10
	end
	return 0
end
...
-- ════════════════════════════════════════════════════════════════
-- ███  SPA COLLISION LOG (sin notificaciones — solo registro)  ████
-- Umbral alto (45 Δv) para capturar solo impactos reales.
-- Las notificaciones push están desactivadas para evitar spam.
-- ════════════════════════════════════════════════════════════════
SPA_Crash = {
	THRESHOLD     = 45,   -- Δv mínimo (studs/s) — solo impactos fuertes reales
	RADIUS        = 28,   -- radio (studs) entre coches para par coche-coche
	COOLDOWN_PAIR = 10,   -- segundos entre el mismo par
	COOLDOWN_WALL = 6,    -- segundos muro por mismo jugador
	MIN_SPEED     = 22,   -- velocidad mínima para considerar impacto
	GRACE_LAP     = 5,    -- segundos de gracia tras cruzar meta
	MAX_LOG       = 60,
	prevVel   = {},
	pairCD    = {},
	wallCD    = {},
...
function _setupCollisionDetection()
	...
	RunService.Heartbeat:Connect(function(dt)
		_tCrash += dt
		if _tCrash < 0.1 then return end   -- 10 Hz — menos carga que antes (era 20 Hz)
		_tCrash = 0
		...
		for uid1, d1 in pairs(impacted) do
			...
			for uid2, d2 in pairs(impacted) do
				...
				-- Sin notificación push — solo log
	...
	local function rebuildChoquesUI()
		...
		if #SPA_Crash.log == 0 then
			...
			lbl.Text="Sin choques registrados aún.\n(Umbral: impactos fuertes ≥45 Δv)"
		...
		for idx, e in ipairs(SPA_Crash.log) do
			...
			tyLbl.Text=(e.wall and "🧱 MURO" or ("💥 "..e.type))
			...
			sevLbl.Text=e.sev; sevLbl.Font=Enum.Font.GothamBlack
		...
		clrBtn.Text="🗑  LIMPIAR HISTORIAL"; clrBtn.Font=Enum.Font.GothamBlack
...
function _setupUI2()  -- [SPAV4] global: libera registros del ámbito principal
	...
	-- ══════════════════════════════════════════════════════════
	-- HB-SPEED (0.06s): nombre + velocidad real + alerta límite
	-- Detecta VehicleSeat Y Seat normal. Sin gate de localChar.
	-- ══════════════════════════════════════════════════════════
	local _tHead = 0
	RunService.Heartbeat:Connect(function(dt)
		_tHead += dt
		if _tHead < 0.06 then return end
		_tHead = 0
		for _, p in ipairs(Players:GetPlayers()) do
			local char = p.Character
			if not char then continue end
			...
			local inVeh = seat
				and (seat:IsA("VehicleSeat") or seat:IsA("Seat"))
			...
			if inVeh then
				...
				if speedLbl then
					...
					-- MaxSpeed del coche (configuración del vehículo)
					local maxSpd = getPlayerSpeedLimit(seat)
					-- Velocidad real actual en studs/s (info aparte)
					...
					if SHOW_HEAD_SPEED then
						...
						-- Rojo cuando el MaxSpeed del coche supera el límite del operador
						speedLbl.TextColor3= maxSpd > effLim and C_RED or C_WHITE
						speedLbl.Visible   = true
					else
						speedLbl.Text = ""; speedLbl.Visible = false
					end
					-- Alerta única cuando el coche tiene un MaxSpeed ilegal
	...
	-- ══════════════════════════════════════════════════════════
	-- HB-TEL (0.25s): turbo + drift + suspensión
	-- Corre más lento que speed — los valores de tel no cambian
	-- a 16 Hz, con 4 Hz es suficiente y ahorra mucho CPU.
	-- ══════════════════════════════════════════════════════════
	local _tTel = 0
	RunService.Heartbeat:Connect(function(dt)
		_tTel += dt
		...
		for _, p in ipairs(Players:GetPlayers()) do
			...
			local seat = humanoid.SeatPart
			if not seat
				or not (seat:IsA("VehicleSeat") or seat:IsA("Seat"))
				or seat.Occupant ~= humanoid then continue end
			local uid = p.UserId
			-- Reconectar monitor de drift si cambió de vehículo
	...
	-- ── HB-LAP (0.1s): detección de vueltas, pits y CC ──────────
	local _tLap  = 0
	RunService.Heartbeat:Connect(function(dt)
		_tLap += dt
		if _tLap >= 0.1 then
			_tLap = 0
			-- Actualizar filtro de overlap para que playersInPart detecte personajes
...
_setupUI2()
_setupCollisionDetection()  -- log de choques (sin notificaciones push)
