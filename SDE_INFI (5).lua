	...
	_SDEI.hbCrash = function()
		...
		for uid1, d1 in pairs(impacted) do
			...
			for uid2, d2 in pairs(impacted) do
				...
				-- Notificación push desactivada — genera spam en móvil con falsos positivos
...
function _setupReplayDetection()
	...
	_SDEI.hbBuf = function()
		for _, p in ipairs(Players:GetPlayers()) do
			...
			local pos = (seat and (seat:IsA("VehicleSeat") or seat:IsA("Seat")) and seat.Occupant == hum)
				and seat.Position
				or hrp.Position
			local vel = (seat and (seat:IsA("VehicleSeat") or seat:IsA("Seat")) and seat.Occupant == hum)
...
function _setupUI2()  -- [SPAV4] global: libera registros del ámbito principal
	...
	_SDEI.hbHead = function()
		-- [FIX LIMPIO] Lógica directa sin caché ni _telGetRootModel
		-- Basada en SPEED_LIMPIO: detecta seat (VehicleSeat o Seat normal),
		-- muestra velocidad real actual en studs/s y km/h, telemetría si aplica.
		for _, p in ipairs(Players:GetPlayers()) do
			local char = p.Character
			if not char then continue end
			...
			if inVehicle then
				...
				-- Velocidad — usa velocidad REAL actual (igual que SPEED_LIMPIO)
				local speedLbl = tag:FindFirstChild("SpeedText")
				if speedLbl and SHOW_HEAD_SPEED then
					local uid      = p.UserId
					local cdS      = customPlayerData[uid]
					local effLim   = (cdS and cdS.speedLimit) or SPEED_LIMIT
					-- Velocidad real actual en studs/s
					local studs    = getVehicleSpeed(seat)
					local kmh      = toSpeedDisplay(studs)
					local dist     = (head.Position - Camera.CFrame.Position).Magnitude
					speedLbl.Text      = sformat("%d km/h  |  %d st/s", kmh, mfloor(studs + 0.5))
					speedLbl.TextSize  = mclamp(30*(10/mmax(dist,1)), 14, 40)
					speedLbl.TextColor3= kmh > effLim and C_RED or C_WHITE
					speedLbl.Visible   = true
					-- Alerta única cuando supera el límite
					if kmh > effLim and not lastSpeeds[uid] then
						lastSpeeds[uid] = true
						showSpeedingNotification(getDisplayName(p), kmh)
					elseif kmh <= effLim then
						lastSpeeds[uid] = nil
					end
				elseif speedLbl then
					speedLbl.Text = ""; speedLbl.Visible = false
				end

				-- Telemetría (turbo/drift/susp) — solo si aplica
				...
				-- Fuera del vehículo: ocultar datos pero mantener tag con nombre
				local tag = char:FindFirstChild("Head") and char.Head:FindFirstChild("SpeedTag")
				if tag then
					local speedLbl = tag:FindFirstChild("SpeedText")
					local telLbl   = tag:FindFirstChild("TelemetryText")
					if speedLbl then speedLbl.Text = ""; speedLbl.Visible = false end
					if telLbl   then telLbl.Text   = ""; telLbl.Visible   = false end
				end
				lastSpeeds[p.UserId] = nil
			end
		end
	end
...
RunService.Heartbeat:Connect(function(dt)
	...
	if HB.crash >= 0.05                  then HB.crash = 0; -- _hbCrash desactivado: fuente de lag, notifs ya silenciadas end
